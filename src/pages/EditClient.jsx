...
const EditClient = () => {
  ...
  const fetchData = async () => {
    try {
      const response = await axios.get(`http://localhost:8080/client/get/${clientId}`);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await axios.put(`http://localhost:8080/client/update/${clientId}`, cardData);
      console.log(response.data);

        navigate("/happyClients");
    ...
    <div className="flex justify-center items-center md:my-5">
        <div className="bg-white shadow-md rounded-md p-4 md:p-6 lg:p-8 flex flex-col items-center w-full md:w-2/3 lg:w-1/2 xl:w-1/3">
          <form onSubmit={handleSubmit} className="w-full">
            <h2 className="text-xl md:text-2xl lg:text-3xl font-bold mb-4 text-center text-blue-500">Edit Client</h2>
            <div className="mb-4">
